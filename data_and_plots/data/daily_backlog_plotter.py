...
# { design: array (samplings, timesteps) }
design_daily = {}
...
for design in sorted(os.listdir(folder_path)):
    ...
    for fname in os.listdir(inner_dir):
        if agents_tag not in fname or not fname.endswith(".csv"):
            continue
        try:
            data = load_backlog_csv(os.path.join(inner_dir, fname))
...
for i, ep in enumerate(sample_labels):
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(PLOT_DIR, f"daily_backlog_ep{ep}_{agents_tag}.pdf"))
    plt.close()
    print(f"saved: daily_backlog_ep{ep}_{agents_tag}.pdf")